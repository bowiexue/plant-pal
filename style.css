...
function convertStr(str,type){
  ...
  for(var i=0;i<str.length;i++){
    ...
    if(mapped && typeof mapped === 'string' && mapped.indexOf('<')!==-1){mapped=mapped.replace(/<[^>]*>/g,'');}
    out+= mapped?mapped:c;
  }return out;
}
...
setInterval(function(){var d=new Date();var el=document.getElementById('wdg-time');if(el)el.innerText=d.toLocaleTimeString()+' ('+d.toLocaleDateString()+')';},1000);
...
function show(){var m=Math.floor(time/60),s=time%60;var el=document.getElementById('tm');if(el)el.innerText=(m<10?'0':'')+m+':'+(s<10?'0':'')+s;}
...
function ren(){
  ...
  var el=document.getElementById('el');if(el)el.innerHTML=out.join('');
  msg();
}
function chg(){
...
function checkWin(){
  ...
  if(pts&&pts%9===0&&!lockGrowth){
    ...
    e.className='pet';e.innerText=currentAnimal;e.style.left=Math.random()*450+'px';var anEl=document.getElementById('an');if(anEl)anEl.appendChild(e);
...
function draw(){
  if(!cx)return;cx.clearRect(0,0,144,144);var l=pts>=9?3:pts>=6?2:pts>=3?1:0;pxl(4,13,8,1,"#c2c5cc");pxl(4,14,8,1,"#8d99ae");pxl(5,12,6,1,'#4a3728');
  
  if(sleep || window.hamsterIsSad){
    pxl(7,10,2,2,'#52b788');cx.fillStyle=(kind=="orchid"||kind=="maple")?'#fff':'#2f3e46';cx.font="12px monospace";cx.fillText("z Z",100,40);
    pxl(9,2,4,1,"#90e0ef");pxl(8,3,6,1,"#00b4d8");return;
  }
  var k=kind;
